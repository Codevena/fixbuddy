#!/usr/bin/env bash
# fixbuddy v0.3.1 — two-agent pipeline for autonomous issue fixing
#
# Pipeline per issue:
...
#   4. PUSH  + PR + auto-merge — CI gates regressions
#
# Crash handling (v0.3):
...
# Filter out fix:applied, fix:false-positive, fix:rejected and any umbrella/meta
...
_Auto-generated by fixbuddy v$VERSION. See fixbuddy-crash/watchdog markers in the run log._" >/dev/null 2>&1 || true
  if [ -n "$branch" ]; then
    ...
    ( cd "$PROJECT" && \
      git stash push --include-untracked -m "fixbuddy-crash-$num-$(ts)" --quiet >/dev/null 2>&1; \
...
_Auto-closed by fixbuddy v$VERSION._" >/dev/null 2>&1 || true
  ...
  while [ "$attempt" -le "$MAX_RETRIES" ]; do
    ...
    if echo "$out" | grep -qE '^DONE-BLOCKED'; then
      ...
      ( cd "$PROJECT" && git checkout "$BASE_BRANCH" >/dev/null 2>&1 && git branch -D "$branch" >/dev/null 2>&1 )
      blocked=$((blocked+1))
      return 0
    fi
...
Full logs: \`$issue_log\` on the machine where fixbuddy ran." >/dev/null 2>&1 || true
  ...
  # ---- Stage 4: PUSH + PR + AUTO-MERGE ----
  info "[#$num] PUSH $branch"
  ( cd "$PROJECT" && git push -u origin "$branch" ) >> "$issue_log" 2>&1 || { err "[#$num] push failed"; rejected=$((rejected+1)); return 0; }
...
Closes #$num" 2>&1) || { err "[#$num] PR creation failed: $pr_url"; rejected=$((rejected+1)); return 0; }
  info "[#$num] PR: $pr_url"

  if $AUTO_MERGE; then
    gh pr merge "$pr_url" --repo "$REPO" --auto --squash --delete-branch >>"$issue_log" 2>&1 \
      || gh pr merge "$pr_url" --repo "$REPO" --squash --delete-branch >>"$issue_log" 2>&1 \
      || warn "[#$num] auto-merge not possible; PR left open"
  fi

  gh issue edit "$num" --repo "$REPO" --add-label "fix:applied" >/dev/null 2>&1 || true
  gh issue comment "$num" --repo "$REPO" --body "**fixbuddy pipeline complete** → $pr_url" >/dev/null 2>&1 || true
  ok "[#$num] merged"
  merged=$((merged+1))
}
